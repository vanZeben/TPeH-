...
public class TeleportAcceptHereCommand implements CommandExecutor {
		...
		if (split.length == 1) {
			...
			if (teleportee != null && teleportee.isOnline()) {
				if (Deity.perm.isAdmin(teleportee) || Deity.perm.isSubAdmin(teleportee)) {
					Deity.chat.sendPlayerError(player, "Teleport", "You cannot summon admins.");
					return false;
				}
				...
				Deity.chat.sendPlayerMessage(teleportee, player.getName() + " has requested that you be teleported to him.");
	...
	public void executeTeleport(Player teleporter, Player teleportee) {
		...
		options.add(new Option("agree", new ConfirmQuestionTask(teleporter, teleportee) {
			@Override
			public void run() {
				if (Deity.player.teleportAsCommand(teleportee, teleporter.getLocation())) {
		...
		options.add(new Option("deny", new ConfirmQuestionTask(teleporter, teleportee) {
			@Override
			public void run() {

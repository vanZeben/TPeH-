...
public class TPeH extends JavaPlugin {
	...
	private boolean checkPlugins() {
		Plugin test = this.getServer().getPluginManager().getPlugin("Questioner");
	...
	private void setupDatabase() {
		if (Deity.data.getDB() != null) {
			Deity.data.getDB().Write("CREATE TABLE IF NOT EXISTS " + Deity.data.getDB().tableName("deity_", "teleports") + " (" + "`id` INT(16) NOT NULL AUTO_INCREMENT ," + "`type` VARCHAR(4) NOT NULL ," + "`teleporter` VARCHAR(16) NOT NULL ," + "`teleportee` VARCHAR(16) NOT NULL ," + "`is_allowed` INT(1) NOT NULL ," + "PRIMARY KEY (`id`)," + "INDEX (`teleporter`)," + "INDEX (`type`)" + ") ENGINE = MYISAM COMMENT =  'Teleporting Record Log';");
		}
	}
